...
int pressnum = A0; // Arduino pin defined on pressure sensor
int tdsnum = A1; // Arduino pin defined on tds sensor
...
{
  char valveData[30];
  sprintf(valveData, "valveNumber=%d&valveStatus=%d", valveNum, valveStat);
  
  //Publish data to Raspberry Pi via MQTT Client named 'client'
  Serial.println(valveData);
  client.publish(valveTopic, valveData);
}
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    // Attempt to connect
    if (client.connect("arduinoClientljsc2222", "mqtt@pi@user1", "user1@pi@123456")) {
...
int calcFlowrate()
{
  ...
  int l_min = (flow_frequency / 29); // (Pulse frequency x 60 min) / 7.5Q = flowrate in L/hour
