...
byte ip[] = {192, 168, 110, 37 }; // ip in lan assigned to arduino
...
char brokerCloud[] = "test.mosquitto.org"; // IP/Domain of cloud MQTT Broker
char brokerPi[] = "192.168.110.36"; // IP of Raspberry Pi MQTT Broker
PubSubClient mqttClient(brokerCloud, 1883, callback, client);
        ...
        if (c == '\n') {
          ...
          if ((readString.indexOf("?button1on") > 0))
          {
            ...
            if (true)
            {
              ...
              while (millis() <= timeMillis + 29000) // Run loop for 29 secs
              {
                SendData();
              ...
              timeMillis = millis();
              while (millis() <= timeMillis + 39000) // Run loop for 39 secs
              {
                SendData();
                delay(2000); // Send Pressure and Flowrate every 2 secs
              }
